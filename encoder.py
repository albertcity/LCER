...
class PixelDecoder(nn.Module):
    ...
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32):
        ...
        self.conv_shape = (num_filters, out_dim, out_dim)
        self.convs = []
        for i in range(num_layers - 1):
          self.convs.append(nn.Conv2dTranspose(num_filters, num_filters, 3, stride=1))
          self.convs.append(nn.ReLU())
        self.convs.append(nn.Conv2dTranspose(num_filters, obs_shape[0], 3, stride=2))
        self.convs = nn.Sequential(*self.convs)
    def forward(self, obs):
        conv_in = self.fc(obs).view(obs.size(0), *self.conv_shape)
        conv_out = self.convs(conv_in)
        conv_out = conv_out * 255
        return convs_out
    ...
    def log(self, L, step, log_freq):
        ...
        for i in range(self.num_layers):
            L.log_param('train_decoder/conv%s' % (i + 1), self.convs[i], step)
