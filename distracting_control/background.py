...
# sess = tf1.Session(config=tfconfig)
...
def size_and_flatten(image, ref_height, ref_width):
  ...
  if image_height != ref_height or image_width != ref_width:
    ...
    # image = tf.image.resize(image, [ref_height, ref_width]), tf.uint8)
    image = Image.fromarray(np.uint8(image)).resize([ref_height, ref_width])
  ...
  def _reset_background(self):
    ...
    if self._video_paths:
      ...
      if self._shuffle_buffer_size:
        ...
        file_names = [
            os.path.join(path, str(fn, encoding='utf-8'))
        ...
        file_names = [str(fn, encoding='utf-8') for fn in file_names]
